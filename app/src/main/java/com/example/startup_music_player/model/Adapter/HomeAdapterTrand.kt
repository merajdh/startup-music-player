...
class HomeAdapterTrand(
    private val data: List<MusicTrendRespomse>,
    private val OnClick: OnClickHome
) : RecyclerView.Adapter<HomeAdapterTrand.HomeViewHolder>() {

    inner class HomeViewHolder(private var binding: ItemRecyclerTrandMusicBinding) :
        ...
        fun ViewBinder(dataHome: MusicTrendRespomse) {
            ...
            if (adapterPosition % 2 == 0) {
                binding.txtNumber.setTextColor(
                    ContextCompat.getColor(
                        binding.root.context,
                        cn.pedant.SweetAlert.R.color.gray_btn_bg_pressed_color
                    )
                )
                binding.backRec.background =
                    ContextCompat.getDrawable(binding.root.context, R.drawable.shape_back_rec_trand)

            } else {
                binding.backRec.background = ContextCompat.getDrawable(
                    binding.root.context,
                    R.drawable.shape_back_rec_trand2
                )
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): HomeAdapterTrand.HomeViewHolder {
        var binding = ItemRecyclerTrandMusicBinding.inflate(
